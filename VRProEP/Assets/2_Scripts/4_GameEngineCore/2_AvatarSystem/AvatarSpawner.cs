    ...
    public static class AvatarSpawner
    {
        ...
        public static GameObject SpawnMotionTracker(bool newTracker)
        {
            ...
            if (AvatarSystem.AvatarType == AvatarType.AbleBodied)
                motionTrackerConfig.SetDeviceIndex(motionTrackerNumber + 4 + 1); // Set hardware device index to follow //Change 4 to other numbers if the hardware connection sequence is changed
            else
                motionTrackerConfig.SetDeviceIndex(motionTrackerNumber + 5 + 1); // Set hardware device index to follow
