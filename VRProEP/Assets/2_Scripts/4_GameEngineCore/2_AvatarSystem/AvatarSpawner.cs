    ...
    public static class AvatarSpawner
    {
        ...
        public static GameObject SpawnMotionTracker(bool newTracker)
        {
            ...
            if (AvatarSystem.AvatarType == AvatarType.AbleBodied)
                motionTrackerConfig.SetDeviceIndex(motionTrackerNumber + 4 - 1 + 1); // Set hardware device index to follow, -1 if headset is conneted through wires not wifi, + n for n extra base station
            else
                motionTrackerConfig.SetDeviceIndex(motionTrackerNumber + 4 - 1 + 1); // Set hardware device index to follow, same as previous, remember to change device index in PlayerPros prefab
