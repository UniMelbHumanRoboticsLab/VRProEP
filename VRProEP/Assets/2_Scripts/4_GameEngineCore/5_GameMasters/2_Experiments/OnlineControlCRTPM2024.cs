...
public class OnlineControlCRTPM2024 : GameMaster
{
    ...
    public override IEnumerator WelcomeLoop()
    {
        ...
        {
            HudManager.DisplayText("Look to the front. Instructions will be displayed there.");
            ...
            yield return WaitForSubjectAcknowledgement(); // And wait for the subject to cycle through them.
            InstructionManager.DisplayText("A break occurs every session" + "\n\n (Press the trigger)");
            yield return WaitForSubjectAcknowledgement(); // And wait for the subject to cycle through them.
        }
    ...
    public override IEnumerator TrainingLoop()
    {
        ...
        if (AvatarSystem.AvatarType == AvatarType.AbleBodied && sessionNumber == 1)
        {
            ...
            //
            // Start position
            InstructionManager.DisplayText("Thanks. Before start a trial, relax your upper arm and elbow downards." + "\n\n (Press the trigger)");
            HudManager.ClearText();
            HudManager.colour = HUDManager.HUDColour.Red;
            yield return WaitForSubjectAcknowledgement(); // And wait for the subject to cycle through them.
    ...
    public override void HandleTaskDataLogging()
    {
        ...
        logData += iterationNumber + ",";  // Make sure you always end your custom data with a comma! Using CSV for data logging.
    ...
    public override bool IsTaskDone()
    {
        ...
        if (dummyControlEnable)
            ...
            tempCompleteFlag = crtManager.TrialComplete || taskTime > maxTaskTime;
